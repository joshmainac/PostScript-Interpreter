...
class Literal(Expr):
    ...
    def evaluate(self, psstacks):
        "TO-DO (part2)"
        #evaluate means conver expressions to actuall values then push them
        #look up dictionary
        #if function aplly the function
        #push numbers,don't push expression ti opstack
        #push int or bool
...
class Name(Expr):
    ...
    #HW5 change it before evaluate functiom add tuple to dic
    #make def find staticLink(self,psstack,functionname):find the index functionname is define
    def staticLink(self,psstacks,myfunc):
        #check top node if function exist if not follow the link
        #psstacks.dictstack[-1][1] will return the top dict
        #mod it
        def staticLink(self,psstacks,myfunc,index):
            mylist = psstacks.dictstack
            ...
            if '/'+myfunc in mylist[index][1]:
                ...
                if index == mylist[index][0]:
                    ...
                    return staticLink(self,psstacks,myfunc,mylist[index][0])  
        ans = staticLink(self,psstacks,myfunc,-1)       
        return ans      
    ...
    def evaluate(self,psstacks):
        ...
        #check b)    
        elif psstacks.builtin_operators.get(self.var_name,0) != 0:
            psstacks.builtin_operators[self.var_name]()

        # elif isinstance(psstacks.dictstack[-1].get("/"+self.var_name,None),FunctionValue):
        #     myvalue = psstacks.dictstack[-1].get("/"+self.var_name)
        #     myvalue.apply(psstacks)

        elif isinstance(psstacks.lookup(self.var_name),FunctionValue):
            mystaticLink = self.staticLink(psstacks,self.var_name)
            #print((self.staticLink(psstacks,self.var_name),self.var_name))
            #psstacks.dictPush(mystaticLink,{})##put tjis in fanction value
            myvalue = psstacks.lookup(self.var_name)
            #myvalue.apply(psstacks)    ##HW5 change it new function apply
            myvalue.apply_function(psstacks,mystaticLink)    ##HW5 change it new function apply
            
            
        elif True:
            #call staticlink()
            #/x 5 def
            #/y {/x {/egg 5 def egg} def x} def not working maybe because of push
            ...
            #lookup
            myvalue = psstacks.lookup(self.var_name)
            # for d in reversed(psstacks):
            #     if d.get('/' + name, None) != None:
            #         myvalue=  d.get('/' + name, None)
            psstacks.opPush(myvalue)
        #check c) -> x  myadd
        # elif psstacks.dictstack.get(self.var_name,0) != 0:
        #     myobj = psstacks.dictstack[self.var_name]
        #     myobj.evaluate()
        ...
        #push str
        #c) value = "add" if value in stack,duc
        #:dict[value]()
        #b)check dict stack,functionvalue aplly method
        pass     
...
class Block(Expr):
    ...
    def evaluate(self, psstacks):
        "TO-DO (part2)"
        #push on opstack as a class function value
        #print as a block value{   }
        #
                #check example in PDF,
        myFunctionValue = FunctionValue(self.value)
        psstacks.opPush(myFunctionValue)
...
class ArrayValue(Value):
    ...
    def apply(self, psstacks):
        #mylist=psParser.read_expr(self.value)
        # push all the pop it 
...
class FunctionValue(Value):
    ...
    #before execute make new tuple
    
    def apply(self, psstacks):
        #use apply_function instead!!!
        # TO-DO in part2
        #unlike array you don't have to push it and evaluate it 

        expr_list = self.body
        
        psstacks.dictPush(0,{})##put tjis in fanction value
        for expr in expr_list:
            expr.evaluate(psstacks) 
        psstacks.dictPop() 
